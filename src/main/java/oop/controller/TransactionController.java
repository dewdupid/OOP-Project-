...
public class TransactionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        //automatically fill up the modification form textfields
    ...
    /**
     * parent dashboard instance into this controller scene.
     * pop-up table directly to the active user's actual stock.
     */
    public void setParentDashboard(DashboardController dashboard) {
        this.mainDashboard = dashboard;

        if (mainDashboard != null &&
                mainDashboard.getInventoryTable() != null) {

            managementTable.setItems(
                    mainDashboard.getInventoryTable().getItems()
            );
        }
    }
    ...
    private void handleAdd() {
        ...
        try {
            ...
            // Scan  specific user's inventory list to prevent ID conflicts
            ...
            mainDashboard.saveInventoryToFile(); // save
            mainDashboard.updDb(); // Refresh UI
            clearFields();
    ...
    private void handleEdit() {
        ...
        try {
            ...
            // Apply updates to the selected object referencer
            target.setName(name);
            target.setCategory(cat);
            target.setQuantity(qty);
            target.setPrice(prc);
            managementTable.refresh();
            if (mainDashboard.getInventoryTable() != null) {
                mainDashboard.getInventoryTable().refresh();
            }
            mainDashboard.saveInventoryToFile(); //  Save modifications to file
    ...
    private void handleDelete() {
        ...
        if (confirm.showAndWait().orElse(ButtonType.NO) == ButtonType.YES) {
            managementTable.getItems().remove(target);// Remove item row directly
            mainDashboard.saveInventoryToFile();//  Update  statistics card
            mainDashboard.updDb();
            clearFields();
